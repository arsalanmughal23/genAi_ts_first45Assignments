...
let personNameWithTitleCase = personName.split(' ').map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ');
console.log(`each-word-with Title Case: ${personNameWithTitleCase}`);
// camelCase
let personNameInCamelCase = personName.split(' ').map((word, index) => {
    return (index > 0)
